...
const REQUIRED_HELP = 'CSV columns: course_code, course_title, credit_units, programme, level, facilitator_email, facilitator_name, start_time, end_time. ' +
  ...
  'The lecture\'s displayed topic is derived from the course automatically.';

export default function LectureImport() {
  const [rows, setRows] = useState([]);
  const [settings, setSettings] = useState({ active_academic_session: '', active_semester: '' });
  ...
  const handleFile = (e) => {
    ...
    Papa.parse(file, {
      ...
      complete: async ({ data }) => {
        ...
        setSettings({ active_academic_session: academicSession, active_semester: semester });
        ...
        const resolved = data.map((row) => {
          ...
          const existingCourse = courses?.find((c) => c.course_code === courseCode);
          const existingLecturer = lecturers?.find((l) => l.email === facilitatorEmail);

          const needsNewCourse = courseCode && !existingCourse;
          const needsNewLecturer = facilitatorEmail && !existingLecturer;

          const courseOk = Boolean(existingCourse || (needsNewCourse && row.course_title?.trim()));
          const facilitatorOk = Boolean(existingLecturer || (needsNewLecturer && row.facilitator_name?.trim()));
          const validTimes = row.start_time && row.end_time && new Date(row.end_time) > new Date(row.start_time);
          ...
          return {
            ...
            reason: !courseOk ? 'course_code / course_title missing or invalid'
              : !facilitatorOk ? 'facilitator_email / facilitator_name missing or invalid'
              : !validTimes ? 'start_time / end_time missing or end_time not after start_time'
              : '',
          };
        });
  ...
  const handleImport = async () => {
    ...
    const newCourseCodes = [...new Set(
      toImport.filter((r) => r.needsNewCourse && !courseIdByCode[r.course_code]).map((r) => r.course_code)
    )];
    for (const code of newCourseCodes) {
      ...
      const { data, error } = await supabase.from('courses').insert({
        ...
        credit_units: sourceRow.credit_units ? Number(sourceRow.credit_units) : null,
        programme: sourceRow.programme || null,
        level: sourceRow.level ? Number(sourceRow.level) : deriveLevelFromCode(code),
    ...
    const newLecturerEmails = [...new Set(
      toImport.filter((r) => r.needsNewLecturer && !lecturerIdByEmail[r.facilitator_email]).map((r) => r.facilitator_email)
    )];
    for (const email of newLecturerEmails) {
      const sourceRow = toImport.find((r) => r.facilitator_email === email);
      const { data, error } = await supabase.functions.invoke('admin-invite-user', {
        body: { email, role: 'lecturer', full_name: sourceRow.facilitator_name.trim() },
      });
      ...
      const batchResults = await Promise.all(
        batch.map(async (row) => {
          ...
          if (!courseId || !facilitatorId) {
            return { row, ok: false, error: 'Course or lecturer could not be resolved/created' };
          }
          const durationMinutes = Math.round((new Date(row.end_time) - new Date(row.start_time)) / 60000);
          const topic = `${row.course_code} — ${courseTitleByCode[row.course_code] || ''}`.trim();
          ...
          return { row, ok: !error, error: error ? await getFunctionErrorMessage(error) : null };
        })
      );
      ...
      <div className="bg-white rounded-[1.1rem] shadow-md p-4">
        ...
        {rows.length > 0 && (
          <>
            <div className="overflow-x-auto mb-4">
              <table className="min-w-full table-auto border-separate border-spacing-y-2 text-sm text-gray-900">
                ...
                <tbody>
                  {rows.map((row, idx) => (
                    <tr key={idx}>
                      <td className="px-4 py-2">{row.course_code} {row.needsNewCourse && <span className="text-blue-500 text-xs">(new)</span>}</td>
                      <td className="px-4 py-2">{row.start_time}</td>
                      <td className="px-4 py-2">{row.facilitator_email} {row.needsNewLecturer && <span className="text-blue-500 text-xs">(new)</span>}</td>
                      <td className="px-4 py-2">
                        {row.matched ? (
                          <span className="text-green-600">Ready</span>
                        ) : (
                          <span className="text-red-500" title={row.reason}>Blocked: {row.reason}</span>
                        )}
                      </td>
                    </tr>
                  ))}
            ...
            >
              {importing ? (progressMessage || 'Importing…') : `Import ${rows.filter((r) => r.matched).length} lectures`}
            </button>
          </>
        )}
        ...
        {results.length > 0 && (
          <div className="mt-4 text-sm">
            ...
            {results.filter((r) => !r.ok).map((r, idx) => (
              <p key={idx} className="text-red-500">{r.row.course_code}: {r.error}</p>
            ))}
          </div>
        )}
      </div>
