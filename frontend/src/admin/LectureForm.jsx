...
export function LectureForm({ onClose, onCreated }) {
  ...
  useEffect(() => {
    const load = async () => {
      const [facilitatorsRes, settingsRes] = await Promise.all([
        supabase.from('profiles').select('id, full_name').eq('role', 'lecturer').eq('status', 'active').order('full_name'),
  ...
  const handleSubmit = async () => {
    ...
    const topic = selectedCourse ? `${selectedCourse.course_code} — ${selectedCourse.course_title}` : '';

    setSubmitting(true);
    setError('');

    const { error: fnError } = await supabase.functions.invoke('zoom-create-meeting', {
      body: {
        course_id: courseId,
    ...
    if (fnError) {
      setError(await getFunctionErrorMessage(fnError, 'Failed to create lecture.'));
      return;
    }
        ...
        <div className="grid grid-cols-1 gap-3">
          ...
          <div className="flex flex-col gap-1">
            ...
            {facilitatorId && courses.length === 0 && (
              <p className="text-xs text-gray-400">This lecturer has no assigned courses yet — assign one from Users first.</p>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-1">
              ...
              <input type="datetime-local" value={startTime} onChange={(e) => handleStartTimeChange(e.target.value)} className="w-full h-11 px-3 border border-gray-200 rounded-xl text-sm" />
            </div>
            <div className="flex flex-col gap-1">
              ...
              <input
                type="datetime-local"
                value={endTime}
                onChange={(e) => { setEndTimeTouched(true); setEndTime(e.target.value); }}
                className="w-full h-11 px-3 border border-gray-200 rounded-xl text-sm"
              />
              <p className="text-[11px] text-gray-400">Auto-set to +2h from start</p>
            </div>
          </div>
