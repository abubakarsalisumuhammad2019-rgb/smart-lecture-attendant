...
const Addstudent = () => {
  ...
  const handleLookup = async () => {
    ...
    if (error || !data) {
      ...
      setLookupMessage("No student account found with this Matric Number. Create their account first from Users → Invite, then come back here to enroll their face.");
      return;
    }
  ...
  return (
    <>
      <div className="bg-white rounded-[1.1rem] shadow-md p-4">
        <h1 className="text-gray-900 font-semibold mb-4 ml-2">Enroll a Student's Face</h1>
        <p className="text-sm text-gray-500 ml-2 mb-4">
          Face enrollment is attached to an existing student account. Look the student
          up by Matric Number first -- if they don't have an account yet, invite them from the
          Users page before coming back here.
        </p>
        ...
        {lookupMessage && (
          <p className="text-sm text-red-500 ml-5 -mt-3 mb-3">{lookupMessage}</p>
        )}

        {foundStudent && (
          <div className="ml-5 mb-3 text-sm text-gray-700">
            <p><strong>Name:</strong> {foundStudent.full_name}</p>
            <p><strong>Face already enrolled:</strong> {foundStudent.face_enrolled ? 'Yes (re-enrolling will overwrite)' : 'No'}</p>
          </div>
        )}
