...
const Enrolled = () => {
    ...
    const fetchStudentsAndAttendance = async () => {
      try {
        const [studentsRes, attendanceRes] = await Promise.all([
          ...
          supabase.from('attendance_logs').select('*').order('recognized_at', { ascending: false }),
        ]);
  ...
  const getAttendanceStatus = (usn) => {
    const hasLogToday = attendanceLogs.some(log => {
      const logDate = new Date(log.recognized_at).toISOString().split("T")[0];
    ...
    <div className="min-h-screen p-4 bg-split relative">
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center">
          <div className="bg-white rounded-xl p-6 w-96 shadow-lg relative">
            ...
            <div className="space-y-2 text-sm text-gray-800">
              ...
              <p><strong>Enrolled At:</strong> {new Date(selectedStudent.enrolled_at).toLocaleDateString()}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="w-full lg:w-64 bg-white shadow-xl rounded-2xl p-4 flex flex-col justify-between min-h-[90vh]">
          ...
          <Link to='/signin'>
            <button onClick={() => supabase.auth.signOut()} className="w-full py-2 rounded-xl bg-[#1E2A78] text-white shadow-md flex items-center justify-center space-x-2 hover:bg-[#16239D]">
              <FaDownload />
              <span>LogOut</span>
            </button>
          </Link>
