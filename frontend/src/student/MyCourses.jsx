...
export default function MyCourses() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setCourses((enrollments || []).map((e) => ({ ...e.courses, source: e.source })).filter((c) => c.id));
      setLoading(false);
    };
    load();
  }, [profile?.id]);
          ...
          <table className="min-w-full table-auto border-separate border-spacing-y-2 text-sm text-gray-900">
            <thead>
              <tr className="bg-[#F7F7F7]">
                ...
                <th className="text-left px-4 py-2 rounded-r-lg">Level</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan="4" className="text-center py-4 text-gray-500">No courses enrolled yet.</td></tr>
              )}
            </tbody>
          </table>
        </div>
